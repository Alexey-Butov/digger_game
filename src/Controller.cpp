...
{
	m_board.load_level(m_current_level, demon_restore);   // load level
	...
	m_background.setTexture(Multimedia_Files::instance().Get_Texture(Multimedia_Files::background));
...
{      
	...
	audio_strike.setBuffer(Multimedia_Files::instance().Get_Audio(Multimedia_Files::strike));
	audio_sound_track.setBuffer(Multimedia_Files::instance().Get_Audio(Multimedia_Files::soundtrack));
	...
	m_new_level = m_menu.start(m_window); // first menu
	...
	while (m_window.isOpen()) // main loop
	{
		...
		if (m_board.get_level_diamonds() == m_board.get_eaten_diamonds()) // finished a level
		{
			Level_clock.restart();  
		...
		while (m_window.pollEvent(Event))
		{
			switch (Event.type)
			{
			case sf::Event::KeyPressed:   // get direction for digger
