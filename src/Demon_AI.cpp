	...
	std::vector< std::vector < std::unique_ptr <Obsticle> > > * obsticle_vec)
{
	...
	while (!m_que.empty() && !found_destination)
	{
		...
		{
			found_destination = check_node(save_start, m_direction[i], obsticle_vec, &map, num_rows, num_cols, i);
	...
	std::vector <std::vector <Node> > *map_ptr, int num_rows, int num_cols, int index)
{
	...
	if (vertex.y >= num_cols || vertex.y < 0 || vertex.x >= num_rows || vertex.x < 0)
		...
		if (obsticle_vec->at(vertex.x).at(vertex.y) != nullptr)
		{
			// check if its a wall or stone if we already visited this location on the map
			if (obsticle_vec->at(vertex.x).at(vertex.y)->get_type() != '#'
				&& obsticle_vec->at(vertex.x).at(vertex.y)->get_type() != '@'
